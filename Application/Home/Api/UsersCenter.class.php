...
class UsersCenter extends Base
{
    ...
    public function addBankCard($param)
    {
        ...
        $data['bank_name'] = $this->bankInfo($param['bank_card']);//银行名称
        $data['user_type'] = $param['user_type'];//用户类型(COM:机构;TEA:老师;STU:学生)
    ...
    public function bankCardList($param)
    {
        ...
        $where['user_type'] = $param['user_type'];//用户类型(COM:机构;TEA:老师;STU:学生)
        $where['audit_status'] = 'S';//审核状态
        $list = D('api_bankcard')->field('id,user_name,phone,bank_name,bank_card')->where($where)->order('id asc')->select();
        if ($list) {
            return array('response_status' => 'success');//success:成功;fail:失败
        } else {
            return array('response_status' => 'fail');//success:成功;fail:失败
    ...
    {
        $data['e_money'] = $param['e_money'];//提现金额
        $data['order'] = date("YmdHis").time().mt_rand(1000,9999);//订单号
        $data['etype'] = 1;//类型(1,消费;2,提现)
    ...
    public function updateMyInfo($param)
    {
        ...
        $data['nick_name'] = $param['nick_name'];//昵称
        $where['id'] =$param['user_id'];//用户id
        $type = $param['user_type'];//用户类型(STU:学生;TEA:老师;COM:机构)
        if($type=='STU'){
            $update = D('api_users')->data($data)->save();
            if ($update) {
                return array('response_status' => 'success');//success:成功;fail:失败
            } else {
                return array('response_status' => 'fail');//success:成功;fail:失败
            ...
            $update = D('api_ct_users')->data($data)->save();
            if ($update) {
                return array('response_status' => 'success');//success:成功;fail:失败
            } else {
                return array('response_status' => 'fail');//success:成功;fail:失败
