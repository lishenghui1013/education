...
class CategoryController extends BaseController {
    ...
    public function ajaxGetIndex()
    {
        ...
        $info = D('api_publish_category as c')->join('left join api_user as u on u.id=c.add_id')->field('c.id,c.category_name,c.del_status,u.username,c.add_time')->where($where)->order('c.id')->limit($start, $limit)->select();
    ...
    public function delete()
    {
        $id = I('post.id');
        $arr['del_status'] = 1;
        $res = D('api_publish_category')->where(array('id' => $id))->save($arr);
        if ($res === false) {
            $this->ajaxError('删除失败');
        } else {
